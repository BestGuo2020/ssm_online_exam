        ...
        elem.off('click').on('click', function(e) {
            ...
            if(opt.searchType == 'more'){
                $.each(opt.searchList, function (index, item) {
                    // 判断字段为班级id
                    if(item.searchKey === 'belongclass') {
                        tableBox += '<input style="display:inline-block;width:190px;height:30px;vertical-align:middle;margin-right:-1px;border: 1px solid #C9C9C9;" type="hidden" name="'+item.searchKey+'" value="' + sessionStorage.getItem("examClassId") + '" placeholder="'+item.searchPlaceholder+'" autocomplete="off" class="layui-input">';
                    } else {
